...
<div class="tab-content">
    <div class="tab-pane" id="inputForm">
        <br>
        <table class="table" id="form_list" align="center" style="width:100%">
            <thead>
    ...
    <div class="tab-pane active" id="onHand">
        <br>
        <input type="button" class="btn btn-success" value="+" style="float: right" id="add">
        <table class="table" id="submission_list" align="center" style="width:100%">
            ...
            <tbody align="center">
                @foreach($submissionsNon as $subm)
                    <tr>
                        <td></td>
                        <td>{{$subm->id}}</td>
                        <td>{{$subm->formNbr}}</td>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var rows_selected = [];
        var stable = $('#submission_list').DataTable({
            columnDefs: [{
                orderable: false,
                targets: 0,
        ...
        var fTable = $('#form_list').DataTable({
            columnDefs: [
                {orderable: false, visible: false, searchable: false, targets: 0},
                {orderable: false, searchable: false, targets: 7, render: function (data, type, full, meta){
                    return '<input type="button" class="btn btn-danger btn-delete" value="X">';
                }},
            ],
            order: [[2, 'desc']],
        });
        ...
        $('#submission_list tbody').on('click', 'input[type="checkbox"]', function(e){
            ...
            updateDataTableSelectAllCtrl(stable);
        ...
        $('thead #select_all', stable.table().container()).on('click', function(e){
            if(this.checked){
                $('tbody input[type="checkbox"]:not(:checked)', stable.table().container()).trigger('click');
            }else{
                $('tbody input[type="checkbox"]:checked', stable.table().container()).trigger('click');
            }
                ...
                $.ajax({
                    ...
                    success: function(response){
                        if(response.error){
                            ...
                            stable.rows('.selected').remove().draw(false);
                            $('#countSelected').empty();
                            rows_selected = [];
                        }
                    }
